...
SELECT COUNT(*) FROM tbl_temp_order;

-- 주문-고객- Relation 생성
SELECT o_num,o_ccode FROM tbl_temp_order;

-- 주문-상품 Relation 생성

SELECT * FROM
(
	SELECT o_num,o_pcode1 AS 상품코드 FROM tbl_temp_order
	UNION
	SELECT o_num,o_pcode2 AS 상품코드 FROM tbl_temp_order
	UNION
	SELECT o_num,o_pcode3 AS 상품코드 FROM tbl_temp_order
) AS M WHERE M.상품코드 <> ''
ORDER BY M.o_num;
...
만약 주문-고객에 코드가 있는데 주문원장에 없다 : 있을수 없다
주문-고객을 기준으로 주문번호가 주문원장에 있는지 확인하기
*/

-- 다음 SQL 의 결과에서 데이터가 1개라도 나오면 
-- FK 설정이 불가능하다
SELECT * 
...
-- 주문원장, 주문-고객 간의 FK 설정
-- CONSTRANINT 이름(fk_oc)를 지정하는 위치 주의!!!
...
-- fk 삭제
ALTER TABLE tbl_order_customer
DROP CONSTRAINT tbl_order_customer_ibfk_3;
...
이처럼 분리된 테이블을 다시 통합 하는 것을 제4정규화, 제5정규화 과정이라고 한다.

tbl_order_product, tbl_order_customer Relation 을 제거하고
주문원장과 고객정보, 주문원장과 상품정보를 연결할수 있는 방법을 모색한다
...
하려고 했더니 
고객은 한 주문에 한명의 고객만 포함되는 원칙이 있어서
문제가 없으나 
...
-- 이미 주문원장과 주문-고객 테이블에 데이터가 많이 있는상태
...
9. SET o_ccode = return(값) 이 실행되며 o_ccode 칼럼값이
	변경된다.
*/
-- MySQL 은 Update,Delete 를 2개이상 레코드(데이터,Row)에
